...
class RequestedReports(models.Model):
    ...
    def remove_cron_records(self):
        for report in self.search([]):
            print ("report -=-=", report, report.state, report.cron_id)
            if report.state == 'complete' and report.cron_id:
                report.cron_id.unlink()

    def download_report(self, report):
        report = self.env['requested.reports'].browse(report)
        adequacies_line_obj = self.env['adequacies.lines']
        bud_line_obj = self.env['expenditure.budget.line']
        report.execution_time = datetime.now()
        ...
        for code in report.program_code_ids:
            col = 0
