...
class MxCONACReportAccountTrial(models.AbstractModel):
    _name = "jt_conac.conac.trial.report"
    _inherit = "account.coa.report"
    _description = "Mexican CONAC Trial Balance Report"

    filter_hierarchy = None

    def _get_reports_buttons(self):
        """Create the buttons to be used to download the required files"""
        buttons = super(MxCONACReportAccountTrial, self)._get_reports_buttons()
    ...
    def _get_templates(self):
        ...
        templates = super(MxCONACReportAccountTrial, self)._get_templates()
    ...
    def _get_lines_second_level(self, lines_child, grouped_accounts,
        ...
        for child in sorted_childs:
            ...
            lines.append({
                ...
                'columns': cols,
                'level': 2,
                'class': 'hierarchy_total' if not options.get('coa_conac_only') else '',
                'unfoldable': True,
                'unfolded': True,
            })
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        return super(MxCONACReportAccountTrial, self.with_context(
    ...
    def open_journal_items(self, options, params):
        ...
        return super(MxCONACReportAccountTrial, self).open_journal_items(
            options, new_params)

    def view_all_journal_items(self, options, params):
        if not params.get('id') or 'hierarchy' in params.get('id'):
            return super(MxCONACReportAccountTrial, self).view_all_journal_items(
                options, params)
        ctx = self._set_context(options)
        ...
        res = super(MxCONACReportAccountTrial, self.with_context(
