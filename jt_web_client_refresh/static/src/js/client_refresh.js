...
odoo.define('jt_web_client_refresh.channel', function (require) {
"use strict";
...
WebClient.include({
	start: function () {
		...
		var load_config = this._rpc({
            route: '/config/jt_web_client_refresh.refresh_delay',
    ...
    refresh: function(message) {
    	...
    	if (!this.call('bus_service', 'isMasterTab') || session.uid !== message.uid && 
            ...
            if (controller.widget.modelName == 'expenditure.budget'){
                this._reload(message, controller);
            }
            if (controller.widget.modelName == 'adequacies'){
                this._reload(message, controller);
            }
            if (controller.widget.modelName == 'standardization'){
                this._reload(message, controller);
            }
            if (controller.widget.modelName == 'control.assigned.amounts'){
                this._reload(message, controller);
            }
